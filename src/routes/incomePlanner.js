...
const SECTOR_MAP = {
  ...
  // Industrials
  'EIF.TO': 'Industrials',
  // Real Estate (REITs use .UN in symbol)
...
function getSector(symbol, name) {
  ...
  // ✅ Try full symbol first (Canada uses full-symbol keys)
...
// ✅ CA added. 'All' replaces the old 'Both' but 'Both' is kept for
// backward compatibility with any cached requests.
...
async function generateAllocation({
  ...
  for (const s of allStocks) {
    ...
    // REITs skip payout-ratio filter (structurally high)
